...
function generateSportProgram() {
 ...
 // ════════════════════════════════════════════════════════════════════════════
 // PIPELINE GÉNÉRATION SÉANCE MUSCU — 8 ÉTAPES STRICTES
 // Déterministe, immutable, UN SEUL TRI (étape 4), jamais re-trié.
 // ════════════════════════════════════════════════════════════════════════════
 dayExercises = (function _buildMuscuDay(grps, cfg) {
   ...
   // ── 1 Pool plat (tous groupes ; exercice annoté avec ses groupes d'appartenance) ──
   var _flat = [], _pidx = {};
   ...
   // ── 2 Filtrage (niveau · équipement · médical · sécurité débutant) ───────
   var _f = _flat.filter(function(ex) {
     ...
     // Grossesse
     if (cfg.pregTri) {
       if (cfg.pregForbidden && cfg.pregForbidden.some(function(f){ return nm.indexOf(f.toLowerCase()) !== -1; })) return false;
     ...
     // Médical questionnaire muscu
     if (cfg.muscuMedical && cfg.muscuMedical.done && typeof filterExerciseByMedical === 'function') {
       if (!filterExerciseByMedical(ex, cfg.muscuMedical)) return false;
     }
     // Médical bridge S.medical
     for (var _ri = 0; _ri < cfg.medRx.length; _ri++) { if (cfg.medRx[_ri].test(nm)) return false; }
     // Débutant : masquer exos exotiques
   ...
   // Fallbacks si pool trop restreint
   if (!_f.length) {
     // Fallback 1 : relâcher filtre équipement=none (pas de machines/câbles)
     if (cfg.equipment === 'none') {
   ...
   if (!_f.length) {
     // Fallback 2 : filtres médicaux seuls
     _f = _flat.filter(function(ex) {
       if (cfg.muscuMedical && cfg.muscuMedical.done && typeof filterExerciseByMedical === 'function')
         return filterExerciseByMedical(ex, cfg.muscuMedical);
       return true;
     });
     if (_f.length) console.warn('[SFC] _buildMuscuDay: fallback médical seul', grps);
   }
   if (!_f.length) {
     // Fallback 3 : pool brut
     _f = _flat.slice();
     console.warn('[SFC] _buildMuscuDay: pool totalement vide, fallback brut', grps);
   }
   ...
   _pool.forEach(function(ex) {
     var tg = ex.tags || [], bw = (_isGym && _isBw(ex)) ? 1 : 0, base;
     if (cfg.isBeginner) {
       // Débutant : machine/guidé→barbell/haltère→poids corps ; +0=composé, +1=autre
       ...
       // Non-débutant : 6 tiers (0=fondamental … 10=finisher) ; bw +1 dans le tier
   ...
   // ── 4 TRI GLOBAL UNIQUE — jamais re-trié après cette ligne ──────────────
   ...
   // ── 5 Sélection coverage-aware ───────────────────────────────────────────
   // Garantit que chaque groupe a ≥1 représentant SANS re-tri ni garde post-sélection.
   // Principe : quand slots restants == groupes non couverts, n'accepter que les exos
   // qui couvrent au moins un nouveau groupe.
   var _sel = [], _sn = {}, _cov = {};
   ...
   for (var _i = 0; _i < _pool.length && _sel.length < cfg.durMax; _i++) {
     ...
     // Slot forcing : ne prendre un exo que s'il couvre du nouveau quand les slots sont comptés
     if (_slots <= _unc && !_e._grps.some(function(g){ return !_cov[g]; })) continue;
     _sel.push(_e); _sn[_k] = true;
     _e._grps.forEach(function(g){ _cov[g] = true; });
     cfg.weekUsed[_k] = true;
   }

   // ── 6 Modificateurs de but (valeurs uniquement — pas de réordonnancement) ─
   _sel = _sel.map(function(ex, idx) {
     ...
     if (cfg.cycleFactor < 0.9 && typeof ex.sets === 'string') {
   ...
   _sel.forEach(function(ex) {
     ...
     if (_dv[k]) console.error('[SFC] Doublon détecté dans la séance:', ex.n, grps);
     _dv[k] = true;
   });
   grps.forEach(function(g) {
     if (!_cov[g]) console.warn('[SFC] Groupe non couvert après sélection:', g, grps);
   });
...
function generateCyclingPlan(level, days) {
 var phases = [
 { name: (window.isEnglish && window.isEnglish() ? 'Aerobic base' : 'Base aérobie'), weeks: [1, 2], color: '#1A3A6A', focus: (window.isEnglish && window.isEnglish() ? 'Build the Z2 engine — quality > quantity' : 'Développer le moteur Z2 — qualité > quantité') },
 { name: (window.isEnglish && window.isEnglish() ? 'Development' : 'Développement'), weeks: [3, 4, 5], color: '#7A3B0E', focus: (window.isEnglish && window.isEnglish() ? 'Introduce tempo and sweet spot' : 'Introduire tempo et sweet spot') },
 { name: (window.isEnglish && window.isEnglish() ? 'Specific' : 'Spécifique'), weeks: [6, 7], color: '#7A1F1F', focus: (window.isEnglish && window.isEnglish() ? 'Threshold intervals and VO2max' : 'Intervalles seuil et VO2max') },
 { name: (window.isEnglish && window.isEnglish() ? 'Taper' : 'Affûtage'), weeks: [8], color: '#3E5C3A', focus: (window.isEnglish && window.isEnglish() ? 'Volume reduction — intensity maintenance' : 'Réduction volume — maintien intensité') }
 ];
