...
function getPregnancySportWarning() {
 ...
 return (window.isEnglish && window.isEnglish()
   ? ' Pregnancy T1 (wk 1-13): Moderate physical activity recommended (150 min/wk, ACOG 2020). Avoid overheating (>38.5°C). Consult your obstetrician before any intensive program.'
   : ' Grossesse T1 (sem. 1-13) : Activité physique modérée recommandée (150 min/sem, ACOG 2020). Évitez le sur-échauffement (>38,5°C). Consultez votre obstétricien avant tout programme intensif.');
 } else if (week < 28) {
 // T2 : veine cave — décubitus dorsal contre-indiqué
 return (window.isEnglish && window.isEnglish()
   ? ' Pregnancy T2 (wk 14-27, ACOG 2020): (1) Avoid prolonged supine position >20 min — inferior vena cava compression. (2) No Valsalva (heavy squat, deadlift) — risk of pressure drop. (3) Contact sports contraindicated. Recommended exercises: swimming, walking, stationary bike, prenatal yoga.'
   : ' Grossesse T2 (sem. 14-27, ACOG 2020) : (1) Évitez le décubitus dorsal prolongé >20 min — compression de la veine cave inférieure. (2) Pas de Valsalva (squat lourd, soulevé de terre) — risque de chute de pression. (3) Sports de contact contre-indiqués. Exercices recommandés : natation, marche, vélo stationnaire, yoga prénatal.');
 } else {
 // T3 : mêmes contre-indications + risque chute de l'équilibre
 return (window.isEnglish && window.isEnglish()
   ? ' Pregnancy T3 (wk 28+, ACOG 2020): (1) Supine position FORBIDDEN. (2) Valsalva forbidden (deadlift, heavy squat, bench press). (3) Contact sports and fall-risk sports contraindicated. (4) Altered balance — prefer guided or supported exercises. Consult your obstetrician before each program modification.'
   : ' Grossesse T3 (sem. 28+, ACOG 2020) : (1) Décubitus dorsal INTERDIT. (2) Valsalva interdit (soulevé de terre, squat lourd, développé couché). (3) Sports de contact et à risque de chute contre-indiqués. (4) Équilibre altéré — préférez exercices guidés ou en appui. Consultez votre obstétricien avant chaque modification du programme.');
 }
}
...
function generateSportProgram() {
 ...
 if (hasShred) {
 restOverride = '45-60s';
 repSuffix = (window.isEnglish && window.isEnglish() ? ' (high intensity)' : ' (haute intensité)');
 supersetNote = (window.isEnglish && window.isEnglish() ? ' — Superset recommended' : ' — Superset recommandé');
 } else if (hasStrength) {
 ...
 if (pregTri) {
 dayExercises.push({
 n: (window.isEnglish && window.isEnglish() ? 'Kegel Exercises' : 'Exercices de Kegel'),
 m: (window.isEnglish && window.isEnglish() ? 'Pelvic floor' : 'Plancher pelvien'),
 sets: '3 x 10 contractions (5s tenue)',
 rest: '30s',
 eq: (window.isEnglish && window.isEnglish() ? 'None' : 'Aucun'),
 lv: 1,
 desc: (window.isEnglish && window.isEnglish() ? 'Contract your pelvic floor muscles as if trying to stop urination. Hold 5 seconds, release 5 seconds. Repeat.' : 'Contractez les muscles du plancher pelvien comme pour retenir l\'urine. Tenez 5 secondes, rel\u00e2chez 5 secondes. R\u00e9p\u00e9tez.'),
 tips: (window.isEnglish && window.isEnglish() ? ['Essential to prepare for childbirth', 'Prevents incontinence', 'Can be done anywhere'] : ['Essentiel pour pr\u00e9parer l\'accouchement', 'Pr\u00e9vient l\'incontinence', 'Peut \u00eatre fait n\'importe o\u00f9']),
 ...
 var _SPLIT_DAY_LABELS = {
   ...
   'ppl_plus1':   ['Push','Pull','Legs','Upper'],
   'bro_4':       (function(){ var _tb = window.isEnglish && window.isEnglish(); return [_tb ? 'Chest + Triceps' : 'Pecs + Triceps', _tb ? 'Back + Biceps' : 'Dos + Biceps', _tb ? 'Shoulders' : 'Épaules', _tb ? 'Legs' : 'Jambes']; })(),
   'ppl_5':       ['Push A','Pull A','Legs','Push B','Pull B'],
   'bro_5':       (function(){ var _tb = window.isEnglish && window.isEnglish(); return [_tb ? 'Chest' : 'Pecs', _tb ? 'Back' : 'Dos', _tb ? 'Shoulders' : 'Épaules', _tb ? 'Arms' : 'Bras', _tb ? 'Legs' : 'Jambes']; })(),
...
function renderSportChoice(p) {
  ...
  var sportLabels = {
    musculation: (_isEN ? 'Weight Training' : 'Musculation'), crossfit: 'Cross Training', running: 'Running',
    hyrox: 'Hyrox', padel: 'Padel', golf: 'Golf', triathlon: 'Triathlon / IRONMAN',
    yoga: (_isEN ? 'Yoga & Mobility' : 'Yoga & Mobilité'), cycling: (_isEN ? 'Cycling' : 'Cyclisme'), calisthenics: (_isEN ? 'Calisthenics' : 'Callisthénie')
  };
  ...
  wrap.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:22px;font-weight:300;font-style:italic;line-height:1.45;margin-bottom:16px'},
    (window.isEnglish && window.isEnglish()
      ? (prenom ? 'Your ' + sportLabel + ' protocol\nis active, ' + prenom + '.' : 'Your ' + sportLabel + ' protocol\nis already set.')
  ...
  wrap.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:13px;letter-spacing:0.5px;line-height:1.7;color:var(--grey,#6B6B65);max-width:300px;margin:0 auto 40px'},
    (window.isEnglish && window.isEnglish()
      ? 'Would you like to continue your current program,\nor define an entirely new one\u00a0?'
...
window.SPORT = {
 ...
 if (_isOnProgram) {
   ...
   var stepLabel = (function(){ var _tsl = window.isEnglish && window.isEnglish(); return S.sStep === 26 ? (_tsl ? 'Health questionnaire' : 'Questionnaire sant\u00e9') : S.sStep === 27 ? (_tsl ? 'Medical check' : 'Bilan m\u00e9dical') : S.sStep === 20 ? (_tsl ? 'Medical eval.' : '\u00c9val. m\u00e9dicale') : S.sStep === 16 ? (_tsl ? 'Load eval.' : '\u00c9val. des charges') : S.sStep === 15 ? (_tsl ? 'Dedicated programs' : 'Programmes d\u00e9di\u00e9s') : (_stepValid ? ((_tsl ? 'Step ' : '\u00c9tape ') + _clampedStep + ' / ' + totalSteps) : '\u2014'); })();
...
function renderSportGenericMedical(p) {
  ...
  p.appendChild(h('div', {style:'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--grey,#6B6B65);text-align:center;margin-bottom:16px;'}, (window.isEnglish && window.isEnglish() ? 'Health Check · ' : 'Bilan Santé · ') + _name));
  ...
  p.appendChild(h('p', {style:'font-family:"Helvetica Neue",Arial,sans-serif;font-size:12px;color:var(--grey,#6B6B65);text-align:center;margin:0 0 24px;max-width:320px;margin-left:auto;margin-right:auto;'}, (window.isEnglish && window.isEnglish() ? 'Flag any sensitive area — your programme will be adapted accordingly. 30 seconds.' : 'Signalez toute zone sensible — votre programme sera adapté en conséquence. 30 secondes.')));
  ...
  }, (window.isEnglish && window.isEnglish() ? (_hasSel ? 'Adapt my programme →' : 'Continue →') : (_hasSel ? 'Adapter mon programme →' : 'Continuer →')));
...
function renderSportSplash(p) {
  ...
  p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'A programme<br><em>made for you.</em>' : 'Un programme<br><em>fait pour vous.</em>')}));
  p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'A few questions to calibrate your programme to your level, goals and health.' : 'Quelques questions pour calibrer votre programme sur votre niveau, vos objectifs et votre santé.')));
  ...
  [
    ...
    (window.isEnglish && window.isEnglish() ? 'Custom-built programme' : 'Programme sur-mesure')
  ...
  var benefits = [
    {icon: '■', title: (window.isEnglish && window.isEnglish() ? 'Science-based programme' : 'Programme scientifique'), desc: (window.isEnglish && window.isEnglish() ? 'Based on ACSM, NSCA and Renaissance Periodization recommendations' : 'Basé sur les recommandations ACSM, NSCA et Renaissance Periodization')},
    ...
    {icon: '◆', title: (window.isEnglish && window.isEnglish() ? 'Adapted to your life' : 'Adapté à votre vie'), desc: (window.isEnglish && window.isEnglish() ? 'Your level, equipment, schedule and goals' : 'Votre niveau, votre équipement, vos disponibilités, vos objectifs')}
  ];
...
function renderObjectif(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Your<br><em>programme</em>' : 'Votre<br><em>programme</em>')}));
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'Choose your sport programme type.' : 'Choisissez votre type de programme sportif.')));
 ...
 (function() {
  ...
  var _isENrec = window.isEnglish && window.isEnglish();
  var _rec = null;
  if (_rAge >= 50) {
   var _50reason = _isENrec ? 'Recommended for 50+ — preserves joints and improves quality of life.' : 'Recommandé pour les 50+ — préserve les articulations et améliore la qualité de vie.';
   if (_rGoalKey === 'cut' || _rGoalKey === 'shred') _50reason = _isENrec ? 'After 50, active yoga promotes gentle fat loss without joint stress — longevity first.' : 'Après 50 ans, le yoga actif favorise la perte de masse grasse douce sans stress articulaire — priorité à la longévité.';
   else if (_rGoalKey === 'bulk' || _rGoalKey === 'lean_bulk') _50reason = _isENrec ? 'After 50, mobility is the foundation — yoga prepares joints before adding load.' : 'Après 50 ans, la mobilité est la base — le yoga prépare les articulations avant d\'ajouter de la charge.';
   _rec = { type: 'yoga', label: (_isENrec ? 'Yoga & Mobility' : 'Yoga & Mobilité'), icon: '🧘',
    sub: (_isENrec ? 'Mobility · Flexibility · Active recovery' : 'Mobilité · Souplesse · Récupération active'),
    reason: _50reason,
    nextStep: 19 };
  } else if (_rGoalKey === 'bulk' || _rGoalKey === 'lean_bulk') {
   _rec = { type: 'musculation', label: (_isENrec ? 'Weight Training' : 'Musculation'), icon: '🏋️',
    sub: (_isENrec ? 'Progressive overload · Strength · Mass' : 'Progressive overload · Force · Masse'),
    reason: (_isENrec ? 'Optimal program for your mass gain goal — scientific progressive overload.' : 'Programme optimal pour votre objectif prise de masse — progressive overload scientifique.'),
    nextStep: 20 };
  } else if (_rGoalKey === 'recomposition') {
   _rec = { type: 'musculation', label: (_isENrec ? 'Weight Training' : 'Musculation'), icon: '🏋️',
    sub: (_isENrec ? 'Strength · Composition · Recomposition' : 'Force · Composition · Recomposition'),
    reason: (_isENrec ? 'Body recomposition responds best to weight training with progressive overload.' : 'La recomposition corporelle répond le mieux à la musculation avec surcharge progressive.'),
    nextStep: 20 };
  } else if (_rGoalKey === 'cut' || _rGoalKey === 'shred') {
   _rec = { type: 'crossfit', label: 'Cross Training', icon: '🔥',
    sub: (_isENrec ? 'HIIT · Strength · Fused cardio' : 'HIIT · Force · Cardio fusionnés'),
    reason: (_isENrec ? 'HIIT + strength = maximum combo for an effective cut — calories burned 24h.' : 'HIIT + force = combo maximal pour une sèche efficace — calories brûlées 24h.'),
    nextStep: 5 };
  } else if (_rGoalKey === 'endurance') {
   _rec = { type: 'running', label: 'Running', icon: '🏃',
    sub: '5K · 10K · Semi · Marathon',
    reason: (_isENrec ? 'Progressive running program — from 5K to marathon with scientific plans.' : 'Programme running progressif — du 5K au marathon avec plans scientifiques.'),
    nextStep: 7 };
  } else if (_isFemale) {
   _rec = { type: 'musculation', label: (_isENrec ? 'Weight Training' : 'Musculation'), icon: '🏋️',
    sub: (_isENrec ? 'Strength · Glutes · Flat stomach' : 'Force · Fessiers · Ventre plat'),
    reason: (_isENrec ? 'Specially adapted for women — glutes and abs priority, without excessive bulk.' : 'Spécialement adapté femme — priorité fessiers et abdominaux, sans bulk excessif.'),
    nextStep: 20 };
  } else {
   _rec = { type: 'musculation', label: (_isENrec ? 'Weight Training' : 'Musculation'), icon: '🏋️',
    sub: (_isENrec ? 'Strength · Volume · Performance' : 'Force · Volume · Performance'),
    reason: (_isENrec ? 'The most complete program to transform your physique durably.' : 'Le programme le plus complet pour transformer votre physique durablement.'),
    nextStep: 20 };
  }
  ...
  recWrap.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;color:var(--grey,#6B6B65);margin-bottom:10px'}, (_isENrec ? 'Recommended for your profile' : 'Recommandé pour votre profil')));
  ...
  var recBadge = h('div', {style: 'position:absolute;top:0;right:0;padding:4px 12px;background:var(--black,#0A0A09);font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--ivory,#FAF9F6);line-height:18px'}, (_isENrec ? '★  Tailored for you' : '★  Adapté à vous'));
  ...
  recCta.appendChild(h('span', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--black,#0A0A09)'}, (_isENrec ? 'Start this program  →' : 'Commencer ce programme  →')));
  ...
  recWrap.appendChild(h('div', {style: 'display:flex;align-items:center;gap:16px;margin-top:20px;margin-bottom:4px'},[
   ...
   h('span', {style:'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--grey,#6B6B65);white-space:nowrap'},(_isENrec ? 'or choose manually' : 'ou choisir manuellement')),
   h('div', {style:'flex:1;height:1px;background:var(--border,#D8D8D0)'})
  ]));
 ...
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Programme type' : 'Type de programme')));
 ...
 h('div', {'class': 'card-name'}, _htaSevere ? (window.isEnglish && window.isEnglish() ? 'Weight Training \u26a0' : 'Musculation \u26a0') : (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation')),
 ...
 h('div', {'class': 'card-tag'}, _htaSevere ? (window.isEnglish && window.isEnglish() ? '\u00c9viter >80% 1RM \u00b7 No Valsalva \u00b7 Consult a doctor' : '\u00c9viter >80% 1RM \u00b7 Pas de Valsalva \u00b7 Consulter un m\u00e9decin') : (window.isEnglish && window.isEnglish() ? 'Cut \u00b7 Mass \u00b7 Strength \u00b7 Endurance' : 'S\u00e8che \u00b7 Masse \u00b7 Force \u00b7 Endurance'))
 ]));
 ...
 var _cfIsEN = window.isEnglish && window.isEnglish();
 var _cfBlockReason = _cfBlocked ? (S.pregnant ? (_cfIsEN ? 'Contraindicated in 3rd trimester (ACOG 2020)' : 'Contre-indiqué pendant le 3e trimestre (ACOG 2020)') : _cfCardio ? (_cfIsEN ? 'Contraindicated with heart disease (AHA 2018)' : 'Contre-indiqué en cas de cardiopathie (AHA 2018)') : (_cfIsEN ? 'Contraindicated with severe HBP (ESC/ESH 2018)' : 'Contre-indiqué avec HTA sévère (ESC/ESH 2018)')) : '';
 ...
 h('div', {'class': 'card-name'}, 'Cross Training' + (_cfBlocked ? (_cfIsEN ? ' — Unavailable' : ' — Indisponible') : '')),
 h('div', {'class': 'card-sub'}, _cfBlocked ? _cfBlockReason : (_cfIsEN ? 'Weightlifting \u00b7 WOD \u00b7 Gymnastics' : 'Halt\u00e9rophilie \u00b7 WOD \u00b7 Gymnastique')),
 h('div', {'class': 'card-tag'}, (window.isEnglish && window.isEnglish() ? '100 WODs \u00b7 6-week Cycles \u00b7 Scaled/Inter/RX' : '100 WODs \u00b7 Cycles 6 semaines \u00b7 Scaled/Inter/RX'))
 ]));
 ...
 h('div', {'class': 'card-sub'}, (window.isEnglish && window.isEnglish() ? 'Technical and physical padel program' : 'Programme technique et physique padel')),
 ...
 h('div', {'class': 'card-sub'}, (window.isEnglish && window.isEnglish() ? 'Improve at golf — Dave Pelz method' : 'Progresser au golf — méthode Dave Pelz')),
 ...
 h('div', {'class': 'card-sub'}, (window.isEnglish && window.isEnglish() ? 'Program Jan Frodeno · Patrick Lange · Daniela Ryf' : 'Programme Jan Frodeno · Patrick Lange · Daniela Ryf')),
 ...
 h('div', {'class': 'card-name'}, (window.isEnglish && window.isEnglish() ? 'Yoga & Mobility' : 'Yoga & Mobilit\u00e9')),
 ...
 h('div', {'class': 'card-name'}, (window.isEnglish && window.isEnglish() ? 'Calisthenics' : 'Callisth\u00e9nie')),
...
function renderPARQ(p) {
 ...
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'Before starting your programme, answer these 7 questions recommended by ACSM (2018). Your answers are confidential.' : 'Avant de démarrer votre programme, répondez à ces 7 questions recommandées par l\'ACSM (2018). Vos réponses sont confidentielles.')));

 // 7 questions PAR-Q
 var PARQ_QUESTIONS = [
  { id: 'cardiac',       text: (window.isEnglish && window.isEnglish() ? 'Has your doctor ever told you that you have a heart problem?' : 'Votre médecin vous a-t-il déjà dit que vous aviez un problème cardiaque ?') },
  { id: 'chestPain',    text: (window.isEnglish && window.isEnglish() ? 'Do you feel chest pain during physical activity?' : 'Ressentez-vous des douleurs thoraciques lors d\'une activité physique ?') },
  { id: 'dizziness',    text: (window.isEnglish && window.isEnglish() ? 'Do you have dizziness or loss of consciousness during exercise?' : 'Avez-vous des étourdissements ou pertes de connaissance lors de l\'effort ?') },
  { id: 'medication',   text: (window.isEnglish && window.isEnglish() ? 'Do you take medication for blood pressure or heart?' : 'Prenez-vous des médicaments pour la tension artérielle ou le cœur ?') },
  { id: 'jointIssue',   text: (window.isEnglish && window.isEnglish() ? 'Do you have a bone or joint problem aggravated by exercise?' : 'Avez-vous un problème osseux ou articulaire aggravé par l\'exercice ?') },
  ...
  { id: 'otherReason',  text: (window.isEnglish && window.isEnglish() ? 'Do you have any other medical reason not to exercise?' : 'Avez-vous une autre raison médicale de ne pas faire d\'activité physique ?') }
 ...
 if (!_hasYes) {
  ...
  warnDiv.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;font-weight:700;color:var(--error,#7A1F1F);margin-bottom:8px;letter-spacing:0.5px'}, (window.isEnglish && window.isEnglish() ? '\u26A0 Consult your doctor before starting this programme.' : '\u26A0 Consultez votre médecin avant de démarrer ce programme.')));
...
function renderMuscuMedicalQ(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation')));
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Medical<br><em>health check</em>' : 'Bilan<br><em>médical muscu</em>')}));
 ...
 if (Array.isArray(S.medical) && S.medical.indexOf('hta_severe') !== -1) {
   ...
   _htaAlert.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;color:#8B0000;line-height:1.6;'}, (window.isEnglish && window.isEnglish() ? 'Loads limited to 40–60% 1RM. Avoid the Valsalva manoeuvre (breath-holding under load), intense isometric movements and overhead exercises at high load. Consult your doctor before starting. — ESC/ESH 2018' : 'Charges limitées à 40–60% du 1RM. Éviter la manœuvre de Valsalva (apnée sous charge), les mouvements isométriques intenses et les exercices au-dessus de la tête à charge élevée. Consultez votre médecin avant de commencer. — ESC/ESH 2018')));
 ...
 if (Array.isArray(window.S && window.S.medical)) {
   ...
   _contextNote.textContent = (window.isEnglish && window.isEnglish() ? 'These questions complement your nutrition profile. They specifically concern your joints and orthopaedic history — distinct from your general medical conditions.' : 'Ces questions compl\u00e8tent votre profil nutrition. Elles portent sp\u00e9cifiquement sur vos articulations et ant\u00e9c\u00e9dents orthop\u00e9diques — diff\u00e9rentes de vos conditions m\u00e9dicales g\u00e9n\u00e9rales.');
 ...
 if (S.muscuMedical && S.muscuMedical.done === true && !S._muscuMedicalEdit) {
  ...
  var _summaryText = _anyPain || _anyDiag
    ? (window.isEnglish && window.isEnglish() ? 'Sensitive areas noted — the programme takes them into account.' : 'Des zones sensibles ont été notées — le programme les prend en compte.')
    : (window.isEnglish && window.isEnglish() ? 'No physical constraints reported. Standard programme.' : 'Aucune contrainte physique signalée. Programme standard.');
 ...
 if ((S.sportLevel === 'beginner' || !S.sportLevel) && !S._muscuMedicalExpanded) {
  ...
  }, (window.isEnglish && window.isEnglish() ? 'Yes, I have something to flag' : 'Oui, j\'ai quelque chose à signaler')));
  filterBtns.appendChild(h('button', {
 ...
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Do you have any pain or fragility?' : 'Avez-vous des douleurs ou fragilités ?')));

 var zonesData = [
 (function(){ var _tZ = window.isEnglish && window.isEnglish(); return [{key: 'shoulders', label: _tZ ? 'Shoulders' : 'Épaules', icon: ''},
 {key: 'elbows', label: _tZ ? 'Elbows' : 'Coudes', icon: ''},
 {key: 'wrists', label: _tZ ? 'Wrists' : 'Poignets', icon: ''},
 {key: 'neck', label: _tZ ? 'Neck/Cervical' : 'Nuque/Cervicales', icon: ''},
 {key: 'upperBack', label: _tZ ? 'Upper back' : 'Haut du dos', icon: ''},
 {key: 'lowerBack', label: _tZ ? 'Lower back' : 'Bas du dos', icon: ''},
 {key: 'hips', label: _tZ ? 'Hips' : 'Hanches', icon: ''},
 {key: 'knees', label: _tZ ? 'Knees' : 'Genoux', icon: ''},
 {key: 'ankles', label: _tZ ? 'Ankles' : 'Chevilles', icon: ''},
 {key: 'feet', label: _tZ ? 'Feet (fasciitis)' : 'Pieds (fasciite)', icon: ''}]; })()
 ];
 ...
 var antecedentsData = [
 (function(){ var _tA = window.isEnglish && window.isEnglish(); return [{key: 'herniaDisc', label: _tA ? 'Disc herniation (MRI)' : 'Hernie discale (IRM)'},
 {key: 'herniaInguinal', label: _tA ? 'Inguinal hernia' : 'Hernie inguinale'},
 {key: 'rotatorCuff', label: _tA ? 'Rotator cuff tear' : 'Déchirure coiffe des rotateurs'},
 {key: 'acl', label: _tA ? 'ACL (operated/fragile)' : 'LCA opéré/fragilisé'},
 {key: 'osteoporosis', label: _tA ? 'Osteoporosis' : 'Ostéoporose'},
 {key: 'hypertension', label: _tA ? 'Severe hypertension (≥180/110)' : 'HTA sévère (≥180/110)'},
 {key: 'rheumatoidArthritis', label: _tA ? 'Rheumatoid arthritis (RA)' : 'Polyarthrite rhumatoïde (PR)'},
 {key: 'fibromyalgia', label: _tA ? 'Fibromyalgia' : 'Fibromyalgie'},
 {key: 'meniscus', label: _tA ? 'Meniscus (torn/operated)' : 'Ménisque lésé/opéré'},
 {key: 'spondylarthritis', label: _tA ? 'Ankylosing spondylitis' : 'Spondylarthrite ankylosante'},
 {key: 'kneeOsteoarthritis', label: _tA ? 'Knee osteoarthritis' : 'Gonarthrose (arthrose du genou)'},
 {key: 'epicondylitis', label: _tA ? 'Lateral epicondylitis (tennis elbow)' : 'Épicondylite latérale (tennis elbow)'}]; })()
 ];
 ...
 var painLevels = [
 (function(){ var _tP = window.isEnglish && window.isEnglish(); return [{val: 0, label: _tP ? 'None' : 'Aucune'},
 {val: 1, label: _tP ? 'Mild' : 'Légère'},
 {val: 2, label: _tP ? 'Moderate' : 'Modérée'},
 {val: 3, label: _tP ? 'Severe' : 'Sévère'}]; })()
 ];
 ...
 placeholder: (window.isEnglish && window.isEnglish() ? 'Add details if needed (e.g. knee surgery 2022, L4-L5 herniation...)' : 'Précisez si besoin (ex: opération genou 2022, hernie L4-L5...)'),
 ...
 warn.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;margin-bottom:6px'}, (window.isEnglish && window.isEnglish() ? 'Warning — Serious history detected' : 'Attention — Antécédent grave détecté')));
 warn.appendChild(h('span', {}, (window.isEnglish && window.isEnglish() ? 'Programme adapted in rehabilitation mode. Consult a doctor or physiotherapist before resuming heavy weight training.' : 'Programme adapté en mode réhabilitation. Consultez un médecin ou kinésithérapeute avant de reprendre la musculation lourde.')));
 p.appendChild(warn);
 }
...
function renderChargesQuestionnaire(p) {
 ...
 if (S.sportLevel === 'beginner' || !S.sportLevel) {
  p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation')));
  p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Ready to<br><em>get started?</em>' : 'Prêt\u00b7e à<br><em>commencer ?</em>')}));
  ...
  reassureCard.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:15px;font-style:italic;margin-bottom:8px;color:var(--black,#1A1A18)'}, (window.isEnglish && window.isEnglish() ? 'Just starting out? Perfect.' : 'Vous débutez ? C\'est parfait.')));
  reassureCard.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:12px;color:var(--grey,#6B6B65);line-height:1.6;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'No need to know your loads. We use your bodyweight as a starting reference and adapt progressively.' : 'Pas besoin de connaître vos charges. Nous utilisons votre poids de corps comme référence de départ et adaptons progressivement.')));
 ...
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'Optional but valuable. Enter the load with which you can do 8-10 clean reps. Leave blank if you do not do the exercise.' : 'Optionnel mais précieux. Indiquez la charge avec laquelle vous faites 8-10 reps propres. Laissez vide si vous ne pratiquez pas l\'exercice.')));
 ...
 h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--orange,#E86F1E);margin-bottom:6px'}, (window.isEnglish && window.isEnglish() ? 'Diabetes — Sport precautions' : 'Diabète — Précautions sportives')),
 (function(){ var _d = h('div', {style: 'color:var(--grey,#6B6B65)'}); _d.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? 'Measure your blood glucose before/after each session. Avoid training if blood glucose < 4.0 mmol/L or > 14.0 mmol/L. Always keep a fast sugar source nearby. Progressive intensity recommended (' : 'Mesurez votre glycémie avant/après chaque séance. Évitez l\'entraînement si glycémie < 4,0 mmol/L ou > 14,0 mmol/L. Gardez toujours une source de sucres rapides à portée de main. Intensité progressive recommandée ('))); _d.appendChild(termTooltip('RPE', _RPE_DEF || 'Rate of Perceived Exertion')); _d.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? ' max 7/10 for the first 4 weeks).' : ' max 7/10 les 4 premières semaines).'))); return _d; })()
 ]));
 }
 if (getAge() >= 50) {
 ...
 h('div', {style: 'color:var(--grey,#6B6B65)'}, (window.isEnglish && window.isEnglish() ? 'Extended 15-20 min warm-up (vs standard 5-10 min). Deload every 4-5 weeks (vs 6 weeks). Prefer guided machines over free bars for max loads. Inter-session recovery 48-72h minimum. Annual medical check recommended.' : 'Échauffement prolongé 15-20 min (vs 5-10 min standard). Décharge toutes les 4-5 semaines (vs 6 semaines). Préférez machines guidées aux barres libres pour les charges maximales. Récupération inter-séance 48-72h minimum. Contrôle médical annuel conseillé.'))
 ]));
 }
 ...
 h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;color:var(--error,#7A1F1F);margin-bottom:6px'}, (window.isEnglish && window.isEnglish() ? 'Pregnancy — Authorised exercises only' : 'Grossesse — Exercices autorisés seulement')),
 h('div', {style: 'color:var(--grey,#6B6B65)'}, (window.isEnglish && window.isEnglish() ? 'Avoid heavy loads, lying on your back (after 20 weeks), high-pressure ab exercises, jumps and intense HIIT. Favour walking, swimming, prenatal yoga, Kegel. Consult your doctor before any training.' : 'Évitez les charges lourdes, exercices allongés sur le dos (après 20 SA), abdominaux hyperpressifs, sauts et HIIT intense. Privilégiez marche, natation, yoga prénatal, Kegel. Consultez votre médecin avant tout entraînement.'))
 ]));
 }
 ...
 _1rmDiv.appendChild(termTooltip('1RM', (window.isEnglish && window.isEnglish() ? '1 Rep Max — the maximum weight you can lift for a single rep.' : 'Répétition Maximale — la charge max que vous pouvez soulever une seule fois')));
 ...
 (function() {
   ...
   var _powerKeys = (function(){ var _tpk = window.isEnglish && window.isEnglish(); return [{ key: 'bench_press', name: _tpk ? 'Bench press' : 'Développé couché' },
     { key: 'squat',       name: 'Squat' },
     { key: 'deadlift',    name: _tpk ? 'Deadlift' : 'Soulevé de terre' }]; })();
   ...
   lbWrap.appendChild(h('div', { style: 'padding:6px 14px;font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;color:var(--grey);font-style:italic;' }, (window.isEnglish && window.isEnglish() ? 'BW = Bodyweight. Estimated 1RM (Epley). General gym population.' : 'PC = Poids de corps. 1RM estimé (Epley). Population générale salle de sport.')));
 ...
 (function() {
   ...
   var _advExercises = [
     { name: (window.isEnglish && window.isEnglish() ? 'Pause Bench Press' : 'Développé couché pause-reps'), unlock: _bench1rm >= (_isFemme ? 0.5 : 0.8) * _bw, req: (_isFemme ? '0.5' : '0.8') + '× ' + (window.isEnglish && window.isEnglish() ? 'BW bench' : 'PC bench'), desc: (window.isEnglish && window.isEnglish() ? '2s pause at bottom to eliminate momentum — pure strength.' : 'Pause 2s en bas pour éliminer l\'élan — force pure.') },
     { name: 'Squat tempo 3-1-3', unlock: _squat1rm >= (_isFemme ? 0.7 : 1.0) * _bw, req: (_isFemme ? '0.7' : '1.0') + '× ' + (window.isEnglish && window.isEnglish() ? 'BW squat' : 'PC squat'), desc: (window.isEnglish && window.isEnglish() ? '3s down, 1s pause, 3s up — maximum control.' : '3s descente, 1s pause, 3s montée — contrôle maximal.') },
     { name: 'Deficit Deadlift (+5cm)', unlock: _dl1rm >= (_isFemme ? 0.9 : 1.3) * _bw, req: (_isFemme ? '0.9' : '1.3') + '× ' + (window.isEnglish && window.isEnglish() ? 'BW DL' : 'PC DL'), desc: (window.isEnglish && window.isEnglish() ? 'Increased range, lower back and hamstring work.' : 'Amplitude accrue, travail du bas du dos et ischio-jambiers.') },
     { name: (window.isEnglish && window.isEnglish() ? 'Weighted Bulgarian Split Squat' : 'Bulgarian Split Squat lesté'), unlock: _squat1rm >= (_isFemme ? 0.6 : 0.9) * _bw, req: (_isFemme ? '0.6' : '0.9') + '× ' + (window.isEnglish && window.isEnglish() ? 'BW squat' : 'PC squat'), desc: (window.isEnglish && window.isEnglish() ? 'Unilateral — balance and leg-by-leg strength.' : 'Unilatéral — équilibre et force jambe par jambe.') },
     { name: 'Paused Squat', unlock: _squat1rm >= (_isFemme ? 0.85 : 1.25) * _bw, req: (_isFemme ? '0.85' : '1.25') + '× ' + (window.isEnglish && window.isEnglish() ? 'BW squat' : 'PC squat'), desc: (window.isEnglish && window.isEnglish() ? '2s pause at bottom — deep base, developed quads.' : 'Pause 2s en bas — base profonde, quadriceps développés.') }
   ];
 ...
 _chargesFooter.appendChild(h('span', {}, (window.isEnglish && window.isEnglish() ? 'Leave blank exercises you do not do. Enter the load AND number of reps for a precise ' : 'Laissez vide les exercices que vous ne pratiquez pas. Indiquez la charge ET le nombre de reps pour un calcul précis du ')));
 _chargesFooter.appendChild(termTooltip('1RM', (window.isEnglish && window.isEnglish() ? '1 Rep Max — the maximum weight you can lift for a single rep.' : 'Répétition Maximale — la charge max que vous pouvez soulever une seule fois')));
...
function appendNutritionModeCTA(p) {
 ...
 card.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;color:var(--success,#3E5C3A);margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'FULL PROGRAMME' : 'PROGRAMME COMPLET')));
 card.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:16px;font-style:italic;margin-bottom:8px;color:var(--black,#1A1A18)'}, (window.isEnglish && window.isEnglish() ? 'Your sport programme is ready.' : 'Votre programme sportif est prêt.')));
...
function renderDedicatedPrograms(p) {
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation')));
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Dedicated<br><em>programmes</em>' : 'Programmes<br><em>dédiés</em>')}));
 ...
 (function() {
   ...
   var _challenges = [
     (function(){ var _tCh = window.isEnglish && window.isEnglish(); return [{ title: _tCh ? '20 extra push-ups' : '20 pompes extra', desc: _tCh ? 'Do it after your session — or separately if no session.' : 'À faire après ta séance — ou séparément si pas de séance.', target: 20, unit: _tCh ? 'push-ups' : 'pompes' },
     { title: _tCh ? '1 min plank' : '1 min de planche', desc: _tCh ? 'Abs tight, body aligned — hold on!' : 'Abdominaux serrés, corps aligné — tiens bon !', target: 60, unit: _tCh ? 'seconds' : 'secondes' },
     { title: _tCh ? '100 jump ropes' : '100 sauts à la corde', desc: _tCh ? 'No rope? Simulate the movement with feet together.' : 'Pas de corde ? Simule le mouvement les pieds joints.', target: 100, unit: _tCh ? 'jumps' : 'sauts' },
     { title: _tCh ? '30 bodyweight squats' : '30 squats au poids du corps', desc: _tCh ? 'Go to parallel, knee aligned with foot.' : 'Descends à parallèle, genou dans l\'axe du pied.', target: 30, unit: 'squats' },
     { title: _tCh ? '15 chair dips' : '15 dips sur chaise', desc: _tCh ? 'Heels on floor, elbows close to body — triceps!' : 'Talons au sol, coudes près du corps — triceps !', target: 15, unit: 'reps' },
     { title: _tCh ? '5 min brisk walk' : '5 min de marche rapide', desc: _tCh ? '100 steps/min minimum — light active cardio.' : '100 pas/min minimum — cardio léger actif.', target: 5, unit: 'minutes' },
     { title: _tCh ? '3 sets of 4-7-8 breathing' : '3 séries de respirations 4-7-8', desc: _tCh ? 'Inhale 4s, hold 7s, exhale 8s — vagal recovery.' : 'Inspire 4s, retiens 7s, expire 8s — récupération vagale.', target: 3, unit: _tCh ? 'sets' : 'séries' }]; })()
   ];
 ...
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Weight training programme' : 'Programme de musculation')));
 p.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;color:var(--grey);margin-bottom:12px'}, (window.isEnglish && window.isEnglish() ? 'Create your custom programme based on your goals, level and target areas.' : 'Créez votre programme sur mesure selon vos objectifs, niveau et zones cibles.')));
 p.appendChild(h('button', {'class': 'btn-primary', onclick: function(){
...
function renderMusculationGoals(p) {
 ...
 banner.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;color:var(--grey,#6B6B65)'}, (window.isEnglish && window.isEnglish() ? 'All changes will be synced with your nutrition plan.' : 'Toute modification sera synchronis\u00e9e avec votre plan nutrition.')));
...
function renderCrossfitLevel(p) {
 ...
 if (S.sex) {
 p.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:11px;color:var(--grey);text-align:center;margin-bottom:16px'}, (window.isEnglish && window.isEnglish() ? 'Adapted loads for: ' : 'Charges adaptées pour : ') + (window.isMale(S) ? (window.isEnglish && window.isEnglish() ? 'Male' : 'Homme') : (window.isEnglish && window.isEnglish() ? 'Female' : 'Femme'))));
 }
 ...
 var _tCF = window.isEnglish && window.isEnglish();
 var explanations = [
 {icon: '', title: 'SCALED', desc: (_tCF ? 'Adapted movements (ring rows, pike push-ups, single unders), light loads. Ideal for starting CrossFit.' : 'Mouvements adaptés (ring rows, pike push-ups, single unders), charges légères. Idéal pour débuter le CrossFit.')},
 {icon: '', title: (_tCF ? 'INTERMEDIATE' : 'INTERMÉDIAIRE'), desc: (_tCF ? 'Full movements with moderate load, some gymnastic adaptations. You have the basics down.' : 'Mouvements complets avec charge modérée, certaines adaptations gymniques. Vous maîtrisez les bases.')},
 {icon: '', title: 'RX', desc: (_tCF ? 'Competition standards, loads and movements at international standard. CrossFit Games level.' : 'Standards compétition, charges et mouvements au standard international. Niveau CrossFit Games.')}
 ];
 ...
 if (S.crossfitLevel) {
 var cfDayReco = '';
 if (S.crossfitLevel === 'scaled') cfDayReco = (window.isEnglish && window.isEnglish() ? 'Recommended: 3-4 days (recovery is important)' : 'Recommand\u00E9 : 3-4 jours (r\u00E9cup\u00E9ration importante)');
...
function renderSportMixSection(p, primarySport) {
 ...
 var compatMap = {
  crossfit:    [
   { type: 'musculation', label: (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation'), desc: (window.isEnglish && window.isEnglish() ? 'Hypertrophy + strength — isolated strength sessions' : 'Hypertrophie + force — séances de renforcement isolées') },
  ...
  musculation: [
   ...
   { type: 'running',     label: 'Running',        desc: (window.isEnglish && window.isEnglish() ? 'Cardio — fat burn + heart health' : 'Cardio — fonte du gras + cœur') },
   { type: 'yoga',        label: 'Yoga',           desc: (window.isEnglish && window.isEnglish() ? 'Mobility + active recovery' : 'Mobilité + récupération active') }
  ],
  running:     [
   { type: 'musculation', label: (window.isEnglish && window.isEnglish() ? 'Weight Training' : 'Musculation'),    desc: (window.isEnglish && window.isEnglish() ? 'Strengthening — injury prevention for running' : 'Renforcement — prévention des blessures course') },
 ...
 p.appendChild(h('div', { style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:12px;color:var(--grey,#6B6B65);margin-bottom:14px;line-height:1.6' }, (window.isEnglish && window.isEnglish() ? 'Add a 2nd sport on your remaining days. The total must equal your weekly target.' : 'Ajoutez un 2\u00e8me sport sur vos jours restants. La somme doit \u00e9galer votre total hebdomadaire.')));
 ...
 p.appendChild(h('div', { 'class': 'section-label', style: 'margin-top:16px' }, (window.isEnglish && window.isEnglish() ? 'Day split' : 'R\u00e9partition des jours')));
...
function renderCFCalendar(p) {
 ...
 var _cfLegEN = window.isEnglish && window.isEnglish();
 var legendItems = [
 {color: '#3E5C3A', label: (_cfLegEN ? 'Completed' : 'Complété')},
 {color: '#1A3C5E', label: (_cfLegEN ? 'Current day' : 'Jour actuel')},
 {color: '#999', label: (_cfLegEN ? 'Upcoming' : 'À venir')}
 ];
 ...
 weeks[fw].push({ day: fd, week: fw, name: (window.isEnglish && window.isEnglish() ? 'DAY ' : 'JOUR ') + fd });
 }
 }
 ...
 // Header semaine
 var _cfCalEN = window.isEnglish && window.isEnglish();
 var weekLabel = (_cfCalEN ? 'Week ' : 'Semaine ') + weekNum;
 // Phases indicatives
 if (weekNum <= 4) weekLabel += ' — Base';
 else if (weekNum <= 7) weekLabel += (_cfCalEN ? ' — Development' : ' — Développement');
 else if (weekNum <= 9) weekLabel += (_cfCalEN ? ' — Intensity' : ' — Intensité');
 else if (weekNum === 10) weekLabel += (_cfCalEN ? ' — Final' : ' — Finale');
...
function renderWellnessCheckin(p, onComplete) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Fitness check-in' : 'Bilan de forme')));
 ...
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'A few questions to adapt your session.' : 'Quelques questions pour adapter votre seance du jour.')));
  ...
  if (window.MOTIVATION) {
    ...
    checkinMotiv.textContent = (window.isEnglish && window.isEnglish() ? 'Taking a minute to assess how you feel is already an act of performance.' : 'Prendre une minute pour évaluer votre état, c\'est déjà un acte de performance.');
 ...
 var muscleOpts = (window.isEnglish && window.isEnglish()) ? [['frais', 'Fresh'], ['courbatures', 'Light soreness'], ['douleurs', 'Real pain']] : [['frais', 'Frais'], ['courbatures', 'Legeres courbatures'], ['douleurs', 'Douleurs reelles']];
 var muscleBtns = [];
...
function appendSportMedicalBanner(p, sportName) {
  ...
  if (Array.isArray(S.medical) && S.medical.length > 0) {
    ...
    if (_ml.indexOf('cardio') !== -1 || _ml.indexOf('insuffisance_card') !== -1) {
      _warns.push((window.isEnglish && window.isEnglish() ? '\u26A0 Heart condition: max Zone 2 intensity (< 65% HRmax). No sprints, no short intervals. Stop immediately if: chest pain, dizziness, unusual breathlessness.' : '\u26A0 Cardiopathie : intensité max Zone 2 (< 65% FCmax). Pas de sprints, pas d\u2019intervalles courts. Arrêtez immédiatement en cas de douleur thoracique, essoufflement anormal ou vertiges (AHA 2018).');
    }
    if (_ml.indexOf('hta') !== -1 || _ml.indexOf('hta_severe') !== -1 || _ml.indexOf('hypertension') !== -1) {
      _warns.push((window.isEnglish && window.isEnglish() ? '\u26A0 Hypertension: avoid intense isometric efforts and sprints. Stay in Zone 1-2. Measure your blood pressure before each session (ESC/ESH 2018).' : '\u26A0 Hypertension : évitez les efforts isométriques intenses et les sprints. Restez en Zone 1-2. Mesurez votre tension avant chaque séance (ESC/ESH 2018).'));
    }
    if (_ml.indexOf('osteoporose') !== -1 || _ml.indexOf('osteoporosis') !== -1) {
      _warns.push((window.isEnglish && window.isEnglish() ? '\u26A0 Osteoporosis: avoid high-impact activities (jumps, sprints, sudden direction changes). Prefer controlled movements and walking (NOF 2022).' : '\u26A0 Ostéoporose : évitez les impacts élevés (sauts, sprints, changements de direction brusques). Privilégiez les mouvements contrôlés et la marche (NOF 2022).'));
    }
    if (_ml.indexOf('diabete_t1') !== -1 || _ml.indexOf('diabete_t2') !== -1) {
      _warns.push((window.isEnglish && window.isEnglish() ? '\u26A0 Diabetes: measure blood glucose before/after. Avoid exercise if < 4 mmol/L or > 14 mmol/L. Keep fast-acting sugar within reach (ADA 2023).' : '\u26A0 Diabète : mesurez votre glycémie avant/après. Évitez l\u2019effort si < 4 mmol/L ou > 14 mmol/L. Gardez du sucre rapide à portée (ADA 2023).'));
    }
    if (_ml.indexOf('polyarthrite') !== -1 || _ml.indexOf('arthrite') !== -1 || _ml.indexOf('rheumatoid') !== -1) {
      _warns.push((window.isEnglish && window.isEnglish() ? '\u26A0 Arthritis: avoid repeated impacts and heavy loads. Prefer gentle movements with controlled range of motion. Stop during inflammatory flares (EULAR 2020).' : '\u26A0 Arthrite : évitez les impacts répétés et les charges lourdes. Privilégiez les mouvements doux à amplitude contrôlée. Arrêtez en cas de poussée inflammatoire (EULAR 2020).'));
    }
    if (_ml.indexOf('fibromyalgie') !== -1) {
      _warns.push((function(){ var _el = document.createElement('span'); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? '\u26A0 Fibromyalgia: light intensity only (' : '\u26A0 Fibromyalgie : intensité légère uniquement ('))); _el.appendChild(termTooltip('RPE', _RPE_DEF || 'Rate of Perceived Exertion')); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? ' max 5/10). Very gradual progression. Avoid exhaustion (Häuser, Cochrane 2017).' : ' max 5/10). Progression très graduelle. Évitez l\u2019épuisement (Häuser, Cochrane 2017).'))); return _el; })());
    }
    if (_warns.length > 0) {
  ...
  if (S.muscuMedical && S.muscuMedical.done) {
    ...
    if (S.muscuMedical.knees || S.muscuMedical.acl || S.muscuMedical.meniscus || S.muscuMedical.kneeOsteoarthritis) {
      _mw.push((window.isEnglish && window.isEnglish() ? '\u26A0 Knees: avoid jumps, sudden direction changes, sprints and downhill. Prefer cycling or swimming.' : '\u26A0 Genoux : évitez les sauts, changements de direction brusques, sprints et descentes. Privilégiez le vélo ou la natation.'));
    }
    if (S.muscuMedical.lowerBack || S.muscuMedical.herniaDisc || S.muscuMedical.spondylarthritis) {
      _mw.push((window.isEnglish && window.isEnglish() ? '\u26A0 Back / Disc: avoid impacts, loaded rotations and prolonged bent positions. Core and mobility work recommended.' : '\u26A0 Dos / Hernie : évitez les impacts, rotations sous charge et positions prolongées penchées. Gainage et mobilité recommandés.'));
    }
    if (S.muscuMedical.shoulders || S.muscuMedical.rotatorCuff) {
      _mw.push((window.isEnglish && window.isEnglish() ? '\u26A0 Shoulders: avoid overhead movements and ballistic gestures (serve, smash, swing).' : '\u26A0 Épaules : évitez les mouvements au-dessus de la tête et les gestes balistiques (service, smash, swing).'));
    }
    if (S.muscuMedical.elbows || S.muscuMedical.epicondylitis) {
      _mw.push((window.isEnglish && window.isEnglish() ? '\u26A0 Elbows / Epicondylitis: avoid repetitive pronation/supination. Protect with an elbow brace.' : '\u26A0 Coudes / Épicondylite : évitez les gestes répétitifs en pronation/supination forcée. Protégez avec une coudière.'));
    }
    if (S.muscuMedical.feet) {
      _mw.push((window.isEnglish && window.isEnglish() ? '\u26A0 Feet / Plantar fasciitis: avoid repeated impacts (running, jumps). Appropriate insoles mandatory.' : '\u26A0 Pieds / Fasciite : évitez les impacts répétés (course, sauts). Semelles adaptées obligatoires.'));
    }
    if (S.muscuMedical.hypertension && _ml.indexOf('hta') === -1 && _ml.indexOf('hta_severe') === -1 && _ml.indexOf('hypertension') === -1) {
      _mw.push((function(){ var _el = document.createElement('span'); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? '\u26A0 Hypertension (muscu profile): no maximal isometric efforts. ' : '\u26A0 HTA (profil muscu) : pas d\u2019efforts isométriques maximaux. '))); _el.appendChild(termTooltip('RPE', _RPE_DEF || 'Rate of Perceived Exertion')); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? ' max 6/10. Stay in Zone 1-2.' : ' max 6/10. Restez en Zone 1-2.'))); return _el; })());
    }
    if (_mw.length > 0) {
...
function appendWellnessBanner(p) {
  ...
  if (score !== null) {
    if (score < 2) {
      ...
      var banner = h('div', {style: 'border-left:3px solid #8B2020;background:rgba(139,32,32,0.06);padding:14px 16px;margin-bottom:16px;border-radius:2px', title: (window.isEnglish && window.isEnglish() ? 'Score based on your sleep, muscle state and energy of the day. Complete your daily assessment to personalise your session.' : 'Score basé sur votre sommeil, état musculaire et énergie du jour. Renseignez votre bilan quotidien pour personnaliser votre séance.')});
      ...
      var banner2 = h('div', {style: 'border-left:3px solid #B8860B;background:rgba(184,134,11,0.06);padding:14px 16px;margin-bottom:16px;border-radius:2px', title: (window.isEnglish && window.isEnglish() ? 'Score based on your sleep, muscle state and energy of the day. Complete your daily assessment to personalise your session.' : 'Score basé sur votre sommeil, état musculaire et énergie du jour. Renseignez votre bilan quotidien pour personnaliser votre séance.')});
      ...
      banner2.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:13px;font-style:italic;color:var(--black,#0A0A09);line-height:1.6;margin-bottom:12px'}, (window.isEnglish && window.isEnglish() ? 'Consider reducing intensity: -1 set per exercise, loads reduced by 10-15%.' : 'Consid\u00e9rez r\u00e9duire l\'intensit\u00e9\u00a0: -1 s\u00e9rie par exercice, charges all\u00e9g\u00e9es de 10-15\u00a0%.')));
      ...
      var badge = h('div', {style: 'display:inline-flex;align-items:center;gap:6px;border:1px solid rgba(26,74,26,0.3);background:var(--paper-2,#F4F1EA);padding:6px 12px;border-radius:2px;margin-bottom:16px;font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--green,#3E5C3A)', title: (window.isEnglish && window.isEnglish() ? 'Score based on your sleep, muscle state and energy of the day. Complete your daily assessment to personalise your session.' : 'Score basé sur votre sommeil, état musculaire et énergie du jour. Renseignez votre bilan quotidien pour personnaliser votre séance.')});
      badge.appendChild(h('span', {}, (window.isEnglish && window.isEnglish() ? 'You are in good shape today' : 'Vous \u00eates en forme aujourd\'hui')));
      p.appendChild(badge);
    }
    return;
  }
  ...
  if (isPeak) {
    ...
    fallbackBanner.appendChild(h('span', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:10px;letter-spacing:2px;text-transform:uppercase;color:var(--success,#3E5C3A)', title: (window.isEnglish && window.isEnglish() ? 'Score based on your sleep, muscle state and energy. Fill in your daily check-in to personalize your session.' : (window.isEnglish && window.isEnglish() ? 'Score based on your sleep, muscle state and energy of the day. Complete your daily assessment to personalise your session.' : 'Score basé sur votre sommeil, état musculaire et énergie du jour. Renseignez votre bilan quotidien pour personnaliser votre séance.'))}, (window.isEnglish && window.isEnglish() ? 'Optimal form — push the loads' : 'Forme optimale — poussez sur les charges')));
  } else {
...
function renderCrossfitProgram(p) {
 ...
 if (_mixSec && S.selectedCrossfitDay >= template.length) {
  ...
  if (_mMixSession) {
   ...
   // Bouton retour
   p.appendChild(h('div', { style: 'height:16px' }));
   p.appendChild(h('button', { 'class': 'btn-back', onclick: function() { S.sStep = 5; window.render(); }, html: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8L10 13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>' + (window.isEnglish && window.isEnglish() ? 'Edit settings' : 'Modifier la configuration') }));
  }
 ...
 if (!wod) { p.appendChild(h('div', {style:'padding:16px;color:var(--grey);font-size:13px'}, (window.isEnglish && window.isEnglish() ? 'Session not available. Reload the page.' : 'Séance non disponible. Rechargez la page.'))); p.appendChild(h('button', {'class':'btn-back', onclick: function(){ S.sStep = 5; window.render(); }}, '← Modifier le niveau')); return; }
 ...
 if (wod.aerobic && wod.aerobic.type) {
  ...
  if (wod.aerobic.rpe !== undefined) {
   (function(){ var _ae = h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:10px;color:#1A5A5A;margin-top:6px'}); _ae.appendChild(termTooltip('RPE', _RPE_DEF || 'Rate of Perceived Exertion')); _ae.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? ' target : ' : ' cible : ') + wod.aerobic.rpe + '/10 — ' + (window.isEnglish && window.isEnglish() ? 'Comfortable Zone 2 effort' : 'Effort Zone 2 confortable'))); aerCard.appendChild(_ae); })();
  }
 ...
 if (wod.recovery && wod.recovery.protocol && wod.recovery.protocol.length) {
  ...
  if (wod.recovery.targets && wod.recovery.targets.length) {
   recCard.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:10px;color:var(--grey);margin-bottom:6px'}, (window.isEnglish && window.isEnglish() ? 'Target areas: ' : 'Zones cibles : ') + wod.recovery.targets.join(', ')));
  }
  var recList = h('div', {});
 ...
 if (S.crossfitLevel === 'rx_plus') {
  ...
  var elitePoints = [
   ...
   (function(){ var _el = document.createDocumentFragment(); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? 'JOURNAL: Record your score, your ' : 'JOURNAL: Notez votre score, votre '))); _el.appendChild(termTooltip('RPE', _RPE_DEF || 'Rate of Perceived Exertion')); _el.appendChild(document.createTextNode((window.isEnglish && window.isEnglish() ? ', and your observations. Progress comes from analysis, not just work.' : ', et vos observations. La progression vient de l\'analyse, pas juste du travail.'))); return _el; })(),
 ...
 var wodMuteBtn = h('button', {'class': 'btn-secondary', style: 'width:auto;padding:8px 12px;margin:0;font-size:10px;letter-spacing:2px;text-transform:uppercase', title: window._sfcMuted ? (window.isEnglish && window.isEnglish() ? 'Sound off — click to enable' : 'Son coupé — cliquer pour activer') : (window.isEnglish && window.isEnglish() ? 'Sound on — click to mute' : 'Son actif — cliquer pour couper'),
 onclick: function() {
 window._sfcMuted = !window._sfcMuted;
 wodMuteBtn.textContent = window._sfcMuted ? (window.isEnglish && window.isEnglish() ? 'Mute' : 'Muet') : (window.isEnglish && window.isEnglish() ? 'Sound' : 'Son');
 wodMuteBtn.title = window._sfcMuted ? (window.isEnglish && window.isEnglish() ? 'Sound off — click to enable' : 'Son coupé — cliquer pour activer') : (window.isEnglish && window.isEnglish() ? 'Sound on — click to mute' : 'Son actif — cliquer pour couper');
 }}, window._sfcMuted ? (window.isEnglish && window.isEnglish() ? 'Mute' : 'Muet') : (window.isEnglish && window.isEnglish() ? 'Sound' : 'Son'));
 ...
 doneCard.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--grey);margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'WOD LOG — Day ' : 'SUIVI WOD — Jour ') + wodDay));
...
function renderMusculationLevel(p) {
 ...
 var _mHint = _mCount === 0
   ? (window.isEnglish && window.isEnglish() ? 'Optional — leave empty for automatic distribution' : 'Optionnel \u2014 laissez vide pour r\u00e9partition automatique')
 ...
 var _teq = window.isEnglish && window.isEnglish();
 var equipOptions = [
 {id: 'gym', label: _teq ? 'Full gym' : 'Salle compl\u00e8te', desc: _teq ? 'Bars, machines, cables' : 'Barres, machines, poulies'},
 {id: 'dumbbells', label: _teq ? 'Dumbbells + bench' : 'Halt\u00e8res + banc', desc: _teq ? 'Home gym or basic gym' : 'Home gym ou salle basique'},
 {id: 'home', label: _teq ? 'Home / BW' : 'Maison / PDC', desc: _teq ? 'No equipment, bodyweight' : 'Sans mat\u00e9riel, poids du corps'}
 ];
 ...
 var _tto = window.isEnglish && window.isEnglish();
 var trainOptions = [
 {id: 'morning', label: _tto ? 'Morning' : 'Matin', desc: _tto ? 'Before 12pm — post-workout breakfast' : 'Avant 12h — petit-d\u00e9j post-s\u00e9ance'},
 {id: 'noon', label: _tto ? 'Noon' : 'Midi', desc: _tto ? '12pm–3pm — post-workout lunch' : '12h\u201315h — d\u00e9jeuner post-s\u00e9ance'},
 {id: 'evening', label: _tto ? 'Evening' : 'Soir', desc: _tto ? 'After 5pm — post-workout dinner' : 'Apr\u00e8s 17h — d\u00eener post-s\u00e9ance'}
 ];
 ...
 p.appendChild(h('div', {'class': 'section-label', style: 'margin-top:24px;'}, (window.isEnglish && window.isEnglish() ? 'Competition to prepare for?' : 'Comp\u00e9tition \u00e0 pr\u00e9parer\u00a0?')));
 ...
 (function(){ var _tc = window.isEnglish && window.isEnglish(); return [{id: false, label: _tc ? 'No' : 'Non', desc: _tc ? 'Standard 12-week program' : 'Programme standard 12 semaines'}, {id: true, label: _tc ? 'Yes \u2014 I have a date' : 'Oui \u2014 j\u2019ai une date', desc: _tc ? 'Targeted periodization' : 'P\u00e9riodisation cibl\u00e9e'}]; })().forEach(function(copt) {
 ...
 p.appendChild(h('div', {'class': 'section-label', style: 'margin-top:24px;'}, (window.isEnglish && window.isEnglish() ? 'Sports practiced alongside?' : 'Sports pratiqu\u00e9s en parall\u00e8le\u00a0?')));
 p.appendChild(h('div', {style: 'font-size:11px;color:var(--grey,#6B6B65);margin:-4px 0 10px;line-height:1.5;'}, (window.isEnglish && window.isEnglish() ? 'To adapt recovery and avoid scheduling conflicts. Optional.' : 'Pour adapter la r\u00e9cup\u00e9ration et \u00e9viter les conflits de charge. Optionnel.')));
 ...
 (function(){ var _th = window.isEnglish && window.isEnglish(); return [{id:'running',lb:'Running'},{id:'cycling',lb:(_th ? 'Cycling' : 'V\u00e9lo')},{id:'swimming',lb:(_th ? 'Swimming' : 'Natation')},{id:'tennis',lb:'Tennis / Padel'},{id:'martialarts',lb:(_th ? 'Martial arts' : 'Arts martiaux')},{id:'ski',lb:'Ski / Snow'},{id:'yoga',lb:'Yoga / Mob'},{id:'other',lb:(_th ? 'Other' : 'Autre')}]; })().forEach(function(hobj) {
 ...
 p.appendChild(h('div', {style: 'font-size:11px;color:var(--grey,#6B6B65);margin-top:4px;'}, (window.isEnglish && window.isEnglish() ? 'No selection = pure weightlifting program' : 'Aucune s\u00e9lection = programme muscu pur')));
...
function renderMusculationZones(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? (_prenom3 ? _prenom3 + ', which zones<br>' : 'Which zones<br>') + '<em>do you want to work\u00a0?</em>' : (_prenom3 ? _prenom3 + ', quelles zones<br>' : 'Quelles zones<br>') + '<em>voulez-vous travailler\u00a0?</em>')}));
 ...
 p.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:11px;color:var(--grey);margin-bottom:12px'}, (window.isEnglish && window.isEnglish() ? 'Assign 1 to 5 stars to set the priority. Click again to remove.' : 'Attribuez 1 à 5 étoiles pour définir la priorité. Cliquez à nouveau pour retirer.')));
 ...
 p.appendChild(h('div', {'class': 'field-error', style: 'text-align:center;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'Choose at least 2 zones to work on to continue.' : 'Choisissez au moins 2 zones à travailler pour continuer.')));
 ...
 if (!_prog || _prog.length === 0 || _prog.every(function(d){ return !d.exercises || d.exercises.length === 0; })) {
   S._programGenerationError = (window.isEnglish && window.isEnglish() ? 'No exercises available with your constraints. Try relaxing your medical restrictions or adding equipment.' : 'Aucun exercice disponible avec vos contraintes. Essayez d\'assouplir vos restrictions médicales ou d\'ajouter de l\'équipement.');
   S.sStep = 4;
   if (window.render) window.render();
   return;
 }
...
(function initRestTimer() {
 ...
 var _trt = window.isEnglish && window.isEnglish(); var labelText = isTrans ? 'Transition' : (_state.exerciseName || (_trt ? 'Rest' : 'Repos'));
 var subText = isTrans
 ? (_state.exerciseName + ' \u2192 ' + (_state.nextExercise || (_trt ? 'Next' : 'Suivant')))
 : ((_trt ? 'Set ' : 'S\u00e9rie ') + _state.setNum + (_trt ? ' done \u2014 rest' : ' termin\u00e9e \u2014 repos'));

 var goText = isTrans
 ? (_state.nextExercise || (_trt ? 'Next exercise' : 'Exercice suivant'))
 : 'GO !';
 ...
 muteBtn.title = window._sfcMuted ? (window.isEnglish && window.isEnglish() ? 'Sound off — click to enable' : 'Son coupé — cliquer pour activer') : (window.isEnglish && window.isEnglish() ? 'Sound on — click to mute' : 'Son actif — cliquer pour couper');
...
function renderWeekTracker(p) {
 ...
 var _macroPhaseLabels = (window.isEnglish && window.isEnglish()) ? ['Hypertrophy','Strength','Transition'] : ['Hypertrophie','Force','Transition'];
 ...
 _macroPhaseLabels.forEach(function(phLabel, idx) {
  var isActive = idx === (cycleNum - 1) % 3;
 ...
 }, (window.isEnglish && window.isEnglish() ? 'Week ' + (week + 1) + ' \u2192' : 'Semaine ' + (week + 1) + ' \u2192')));
 } else {
 btnRow.appendChild(h('button', {
...
function renderMusculationProgram(p) {
 ...
 if (S.muscuMedical && S.muscuMedical.done) {
 var med = S.muscuMedical;
 var _tmed = window.isEnglish && window.isEnglish();
 var restrictions = [];
 if (med.shoulders || med.rotatorCuff) restrictions.push(_tmed ? '\u26A0 Shoulders\u00a0: overhead exercises avoided' : '\u26A0 \u00c9paules\u00a0: exercices overhead \u00e9vit\u00e9s');
 if (med.lowerBack || med.herniaDisc) restrictions.push(_tmed ? '\u26A0 Back\u00a0: deadlifts and heavy bending removed' : '\u26A0 Dos\u00a0: soulev\u00e9 de terre et flexions lourdes retir\u00e9s');
 if (med.knees || med.acl) restrictions.push(_tmed ? '\u26A0 Knees\u00a0: deep squats replaced' : '\u26A0 Genoux\u00a0: squats profonds remplac\u00e9s');
 if (med.herniaInguinal) restrictions.push(_tmed ? '\u26A0 Inguinal hernia\u00a0: hyperpressure exercises removed' : '\u26A0 Hernie inguinale\u00a0: exercices hyperpressifs retir\u00e9s');
 ...
 if (med.osteoporosis) restrictions.push(_tmed ? '\u26A0 Osteoporosis\u00a0: loads \u2264\u00a070\u00a0% 1RM, no impacts or spinal flexions (Sinaki, Spine 2002)' : '\u26A0 Ost\u00e9oporose\u00a0: charges \u2264\u00a070\u00a0% 1RM, pas d\'impacts ni flexions vert\u00e9brales (Sinaki, Spine 2002)');
 if (med.rheumatoidArthritis) restrictions.push(_tmed ? '\u26A0 Rheumatoid arthritis\u00a0: light loads, gentle exercises in remission only \u2014 stop during flare-ups (EULAR 2020)' : '\u26A0 Polyarthrite rhumato\u00efde\u00a0: charges l\u00e9g\u00e8res, exercices doux en r\u00e9mission uniquement \u2014 arr\u00eatez en cas de pouss\u00e9e (EULAR 2020)');
 if (med.fibromyalgia) restrictions.push(_tmed ? '\u26A0 Fibromyalgia\u00a0: moderate intensity max, no HIIT or max loads \u2014 gentle aerobic exercises recommended (Cochrane 2017)' : '\u26A0 Fibromyalgie\u00a0: intensit\u00e9 mod\u00e9r\u00e9e max, pas de HIIT ni charges maximales \u2014 exercices a\u00e9robies doux recommand\u00e9s (Cochrane 2017)');
 if (med.meniscus) restrictions.push(_tmed ? '\u26A0 Meniscus\u00a0: no flexion >90\u00b0 under load or rotational shear (leg extension, lunges)' : '\u26A0 M\u00e9nisque\u00a0: pas de flexion >90\u00b0 sous charge ni de cisaillement en rotation (leg extension, fentes)');
 if (med.feet) restrictions.push(_tmed ? '\u26A0 Feet/fasciitis\u00a0: impact exercises removed (jumps, rope), prefer cycling or swimming' : '\u26A0 Pieds/fasciite\u00a0: exercices \u00e0 impact retir\u00e9s (sauts, corde), privil\u00e9gier velo ou natation');
 if (med.spondylarthritis) restrictions.push(_tmed ? '\u26A0 Ankylosing spondylitis\u00a0: heavy axial loads removed (deadlift, barbell squat, good morning) \u2014 swimming, yoga and stretching recommended (Sieper & Poddubnyy, Lancet 2017)' : '\u26A0 Spondylarthrite\u00a0: charges axiales lourdes retir\u00e9es (deadlift, squat barre, good morning) \u2014 natation, yoga et \u00e9tirements recommand\u00e9s (Sieper & Poddubnyy, Lancet 2017)');
 if (med.kneeOsteoarthritis) restrictions.push(_tmed ? '\u26A0 Knee osteoarthritis\u00a0: deep knee bends and impacts removed \u2014 stationary bike and limited-range resistance training recommended (OARSI 2014)' : '\u26A0 Gonarthrose\u00a0: flexions profondes du genou et impacts retir\u00e9s \u2014 v\u00e9lo stationnaire et musculation en amplitude limit\u00e9e recommand\u00e9s (OARSI 2014)');
 if (med.epicondylitis || med.elbows) restrictions.push(_tmed ? '\u26A0 Epicondylitis\u00a0: pronated barbell rows, pronated pull-ups, straight bar curls removed \u2014 prefer supinated or neutral grip (Bisset & Vicenzino, JOSPT 2015)' : '\u26A0 \u00c9picondylite\u00a0: rowing barre pronation, pull-ups pronation, curl barre droite retir\u00e9s \u2014 favoriser prise supination ou neutre (Bisset & Vicenzino, JOSPT 2015)');
 ...
 medBanner.appendChild(h('div', {style: 'font-weight:bold;margin-bottom:6px'}, (_tmed ? 'Program adapted to your medical profile' : 'Programme adapt\u00e9 \u00e0 votre bilan m\u00e9dical')));
     ...
     _phaseExos.forEach(function(exo) {
      ...
      if (exo.type) {
       ...
       if (exo.type === 'compound') {
        _typeBadge.appendChild(termTooltip('Compound', (window.isEnglish && window.isEnglish() ? 'Compound exercise — recruits multiple muscle groups simultaneously.' : 'Exercice poly-articulaire qui recrute plusieurs groupes musculaires simultanément')));
       } else if (exo.type === 'isolation') {
        _typeBadge.appendChild(termTooltip('Isolation', (window.isEnglish && window.isEnglish() ? 'Isolation exercise — targets a single muscle with one joint.' : 'Exercice mono-articulaire ciblant un seul muscle')));
       } else {
      ...
      if (exo.rirTarget !== undefined) {
       ...
       _rirBadge.appendChild(termTooltip('RIR', (window.isEnglish && window.isEnglish() ? 'Reps In Reserve — number of reps you could still do before muscular failure.' : 'Reps In Reserve — nombre de reps que vous pourriez encore faire avant l\'échec musculaire')));
 ...
 p.appendChild(h('div', {'class': 'section-label', style: 'margin-top:28px'}, (window.isEnglish && window.isEnglish() ? 'Targeted programs' : 'Programmes ciblés'));ppendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;color:var(--grey);margin-bottom:12px'}, (window.isEnglish && window.isEnglish() ? 'Bonus sessions to add based on your priorities.' : 'Séances bonus à intégrer selon vos priorités.')));
 ...
 window.render();
 }}, (window.isEnglish && window.isEnglish() ? '\u21bb Recalculate weekly program' : '\u21bb Recalculer le programme hebdomadaire')));

 // Export PDF
 p.appendChild(h('button', {'class': 'btn-primary', style: 'margin-top:12px;background:var(--black2)', onclick: function() { if (typeof window.exportSportPDF === 'function') window.exportSportPDF(); else if (window.showToast) window.showToast((window.isEnglish && window.isEnglish()) ? 'PDF export not available.' : 'Export PDF non disponible.', 'info', 3000); }}, (window.isEnglish && window.isEnglish() ? '\u21e9 Export program to PDF' : '\u21e9 Exporter le programme en PDF')));
 ...
 pregSportCard.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:18px;color:' + triSportColor + ';margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? '\uD83E\uDD30 Adapted pregnancy program \u2014 ' + (triSport.trimester.name || '') : '\uD83E\uDD30 Programme adapt\u00e9 grossesse \u2014 ' + (triSport.trimester.name || ''))));
 pregSportCard.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:13px;color:var(--grey);margin-bottom:10px'}, (window.isEnglish && window.isEnglish() ? 'Intensity: ' + intensitySport + '% \u2014 Listen to your body' : 'Intensit\u00e9 : ' + intensitySport + '% \u2014 \u00c9coutez votre corps')));
 ...
 pregSportCard.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:4px;text-transform:uppercase;color:' + triSportColor + ';margin:10px 0 6px'}, (window.isEnglish && window.isEnglish() ? 'Exercises forbidden this trimester' : 'Exercices interdits ce trimestre')));
 ...
 pregStopWarn.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:11px;color:' + triSportColor + ';font-weight:bold'}, (window.isEnglish && window.isEnglish() ? '\u26A0 Stop immediately if: bleeding, dizziness, contractions, pain, excessive breathlessness' : '\u26A0 Arr\u00eatez imm\u00e9diatement si : saignements, vertiges, contractions, douleurs, essoufflement excessif')));
 ...
 warnDiv.textContent = (window.isEnglish && window.isEnglish() ? '\u26A0 Recovery phase \u2014 adjust your effort' : '\u26A0 Phase de r\u00e9cup\u00e9ration \u2014 adaptez votre effort');
 ...
 creatTip.appendChild(h('span', {}, creatDose + (window.isEnglish && window.isEnglish() ? 'g after training with carbs for optimal absorption' : 'g apr\u00e8s l\'entra\u00eenement avec glucides pour absorption optimale')));
 ...
 p.appendChild(h('div', {style: 'height:12px'}));
 p.appendChild(h('button', {'class': 'btn-back', onclick: function(){ S.sStep = 3; window.render(); }, html: '<svg width="16" height="16" viewBox="0 0 16 16" fill="none"><path d="M10 3L5 8L10 13" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/></svg>' + (window.isEnglish && window.isEnglish() ? 'Edit zones' : 'Modifier les zones')}));
 appendNutritionModeCTA(p);
}
...
function renderWeightChartSport(container) {
 ...
 if (history.length < 2) {
 section.appendChild(h('div', {style: 'text-align:center;padding:20px;font-family:Helvetica Neue,Arial,sans-serif;font-size:11px;color:var(--grey)'}, (window.isEnglish && window.isEnglish() ? 'Log your weight regularly to see your progress curve' : 'Enregistrez votre poids régulièrement pour voir votre courbe')));
 }
 ...
 if (_hist30.length >= 2) {
   ...
   if (_pastValues.length > 0) {
     ...
     if (_isLoss && v < _pastMin) _prWeightMsg = (window.isEnglish && window.isEnglish() ? 'New monthly low: ' : 'Nouveau record bas ce mois : ') + (window.UNITS ? window.UNITS.displayWeight(v) : v + ' kg') + ' \uD83C\uDFC6';
     else if (_isGain && v > _pastMax) _prWeightMsg = (window.isEnglish && window.isEnglish() ? 'New monthly high: ' : 'Nouveau record haut ce mois : ') + (window.UNITS ? window.UNITS.displayWeight(v) : v + ' kg') + ' \uD83D\uDCAA';
   }
 }
...
function renderSportModal(app) {
 ...
 body.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Working load' : 'Charge de travail')));
...
function renderRunningConfig(p) {
 ...
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'Programming based on Jack Daniels & Pfitzinger method' : 'Programmation basée sur la méthode Jack Daniels & Pfitzinger')));

 // Goal selection (mandatory)
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Running goal' : 'Objectif de course')));
 ...
 h('div', {'class': 'card-tag'}, (window.isEnglish && window.isEnglish() ? g.weeks + ' weeks · Long run max ' + g.longRunMax + 'km' : g.weeks + ' semaines · SL max ' + g.longRunMax + 'km'))
 ]));
 });
...
function renderRunningProgram(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, 'Programme'));
 p.appendChild(h('h1', {html: (goalObj2 ? goalObj2.name : 'Running') + '<br><em>' + (window.isEnglish && window.isEnglish() ? 'Plan' : 'Plan') + '</em>'}));
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? totalWeeks + ' weeks · ' + S.runningDays + ' days/week · Level ' : totalWeeks + ' semaines · ' + S.runningDays + ' jours/semaine · Niveau ') + (levelObj ? levelObj.name : '')));
 ...
 var phaseNamesEN = {Base: 'Base', 'Développement': 'Development', 'Spécifique': 'Specific', 'Affûtage': 'Taper'};
 var phaseColor = phaseColors[currentWeekData.phase] || '#0A0A09';
 var phaseLabelRun = (window.isEnglish && window.isEnglish()) ? (phaseNamesEN[currentWeekData.phase] || currentWeekData.phase) : currentWeekData.phase;
 var infoCard = h('div', {style: 'border-left:3px solid ' + phaseColor + ';padding:12px 16px;background:var(--ivory2);margin-bottom:16px'});
 infoCard.appendChild(h('div', {style: 'font-family:Georgia;font-size:16px;color:' + phaseColor + ';margin-bottom:4px'}, (window.isEnglish && window.isEnglish() ? 'Phase: ' : 'Phase : ') + phaseLabelRun));
  ...
  if (_existing) {
    ...
    var _saveRunBtn = h('button', {style: 'width:100%;padding:12px;background:var(--black,#0A0A09);color:var(--ivory,#FAF9F6);border:none;font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:3px;text-transform:uppercase;cursor:pointer',
      onclick: function() {
        ...
        if (window.showToast) window.showToast((window.isEnglish && window.isEnglish() ? '✓ Run saved — ' + _entry.distanceKm + ' km at ' + _paceStr + ' /km' : '✓ Course enregistrée — ' + _entry.distanceKm + ' km à ' + _paceStr + ' /km'), 'success');
        window.render();
      }
    }, (window.isEnglish && window.isEnglish() ? '✓ Save run' : '✓ Enregistrer la course'));
...
function renderHyroxConfig(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Preparation<br><em>Hyrox</em>' : 'Préparation<br><em>Hyrox</em>')}));
 ...
 p.appendChild(h('div', {style: 'height:24px'}));
 p.appendChild(h('div', {style: 'width:100%;height:1px;background:var(--border);margin-bottom:16px'}));
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Current benchmarks (optional)' : 'Benchmarks actuels (optionnel)')));
 p.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:11px;color:var(--grey);margin-bottom:16px'}, (window.isEnglish && window.isEnglish() ? 'Enter your current times to track your progress.' : 'Renseignez vos temps actuels pour suivre votre progression.')));
 ...
 nameDiv.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:9px;color:var(--grey)'}, (window.isEnglish && window.isEnglish() ? 'Standard ' : 'Standard ') + S.hyroxLevel + ' : ' + station.standards[S.hyroxLevel]));
 }
 row.appendChild(nameDiv);
...
function renderHyroxProgram(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Hyrox Program' : 'Programme Hyrox')));
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Preparation<br><em>12 weeks</em>' : 'Préparation<br><em>12 semaines</em>')}));
 ...
 banner.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:18px;color:var(--error,#7A1F1F);margin-bottom:4px'}, (window.isEnglish && window.isEnglish() ? ' RACE WEEK — GO TIME' : ' RACE WEEK — GO TIME')));
 ...
 valSpan.appendChild(h('span', {style: 'color:' + diffColor + ';margin-left:8px;font-size:11px'}, diffStr + (window.isEnglish && window.isEnglish() ? ' vs standard' : ' vs standard')));
 }
 }
 ...
 (function() {
  ...
  _hxCard.appendChild(h('div', {style: 'font-family:"Helvetica Neue",Arial,sans-serif;font-size:9px;letter-spacing:2px;text-transform:uppercase;color:var(--grey);margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'TRACKING — ' : 'SUIVI — ') + _hxDayKey));
  if (_hxDone) {
    ...
    var _hxDoneBtn = h('button', {'class': 'btn-primary', style: 'width:100%;margin:0', onclick: function() {
      ...
      if (window.showToast) window.showToast((window.isEnglish && window.isEnglish() ? '✓ Hyrox session complete — ' : '✓ Séance Hyrox validée — ') + _hxDayKey, 'success');
      window.render();
    }}, (window.isEnglish && window.isEnglish() ? '✓ Session complete' : '✓ Séance terminée'));
...
function renderPadelConfig(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Your<br><em>padel</em> program' : 'Votre programme<br><em>padel</em>')}));
...
function renderPadelProgram(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Padel Program' : 'Programme Padel')));
...
function renderGolfConfig(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Your<br><em>golf</em> program' : 'Votre programme<br><em>golf</em>')}));
 p.appendChild(h('p', {'class': 'subtitle'}, (window.isEnglish && window.isEnglish() ? 'Based on Dave Pelz & Butch Harmon methods.' : 'Basé sur les méthodes Dave Pelz & Butch Harmon.')));
 ...
 // Handicap
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Current handicap (optional)' : 'Handicap actuel (optionnel)')));
...
function renderGolfProgram(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Golf Program' : 'Programme Golf')));
...
function renderTriathlonConfig(p) {
 ...
 [(function(){ var _t = window.isEnglish && window.isEnglish(); return [{id: 'swim', name: (_t ? ' Swimming' : ' Natation')}, {id: 'bike', name: (_t ? ' Bike' : ' Vélo')}, {id: 'run', name: (_t ? ' Running' : ' Course à pied')}]; })().forEach(function(d) {
 ...
 p.appendChild(h('div', {style: 'height:20px'}));
 p.appendChild(h('div', {style: 'width:100%;height:1px;background:var(--border);margin-bottom:16px'}));
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Current paces (optional)' : 'Allures actuelles (optionnel)')));
 ...
 bikeWrap.appendChild(h('div', {style: 'font-size:11px;color:var(--grey)'}, (window.isEnglish && window.isEnglish() ? ' Bike km/h' : ' Vélo km/h')));
 ...
 p.appendChild(h('div', {style: 'height:16px'}));
 p.appendChild(h('div', {style: 'width:100%;height:1px;background:var(--border);margin-bottom:16px'}));
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Race date (optional)' : 'Date de course (optionnel)')));
...
function renderTriathlonProgram(p) {
 ...
 var subtitleParts = [(window.isEnglish && window.isEnglish() ? totalWeeks + ' weeks' : totalWeeks + ' semaines'), (levelObj ? levelObj.name : ''), '80/20', (window.isEnglish && window.isEnglish() ? 'Jan Frodeno method' : 'Méthode Jan Frodeno')];
 ...
 if (zref && (zref.swim || zref.bike || zref.run)) {
  ...
  zoneCard.appendChild(h('div', {style: 'font-family:Georgia,serif;font-size:13px;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'Your personal zones' : 'Vos zones personnalisées')));
 ...
 phaseCard.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:13px;color:var(--grey)'}, '~' + weekData.totalHours + (window.isEnglish && window.isEnglish() ? ' of training · 7 days' : ' d\'entraînement · 7 jours')));
 ...
 if (weekData.isTaper) phaseCard.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:11px;color:var(--success,#3E5C3A);margin-top:6px;font-weight:bold'}, (window.isEnglish && window.isEnglish() ? ' Taper — Reduced volume, maintained intensity' : ' Affûtage — Volume réduit, intensité maintenue')));
 ...
 if (swCount) sumRow.appendChild(h('span', {'class': 'chip on'}, ' ' + swCount + (window.isEnglish && window.isEnglish() ? ' swim' : ' nage')));
 if (biCount) sumRow.appendChild(h('span', {'class': 'chip on'}, ' ' + biCount + (window.isEnglish && window.isEnglish() ? ' bike' : ' vélo')));
 ...
 p.appendChild(h('div', {style: 'height:12px'}));
 p.appendChild(h('button', {'class': 'btn-back', onclick: function() { S.sStep = 17; window.render(); }, html: backArrow + (window.isEnglish && window.isEnglish() ? 'Edit settings' : 'Modifier la configuration')}));
 appendNutritionModeCTA(p);
}
...
function renderYogaOnboarding(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Your<br><em>yoga</em> program' : 'Votre programme<br><em>yoga</em>')}));
 ...
 if (!ok) {
 p.appendChild(h('div', {'class': 'field-error', style: 'text-align:center;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'Select a level and a goal' : 'S\u00e9lectionnez un niveau et un objectif')));
 }
 p.appendChild(h('button', {'class': 'btn-primary', disabled: !ok, onclick: function(){
 if (ok) {
...
function renderYogaProgram(p) {
 ...
 p.appendChild(h('div', {'class': 'eyebrow'}, (window.isEnglish && window.isEnglish() ? 'Yoga Program' : 'Programme Yoga')));
 ...
 sessCard.appendChild(h('div', {style: 'font-family:Georgia;font-size:18px;margin-bottom:4px'}, (window.isEnglish && window.isEnglish() ? '\uD83E\uDDD8 Session ' : '\uD83E\uDDD8 S\u00e9ance ') + (S.yogaDay + 1) + ' \u2014 ' + weekData.phase));
 ...
 benefDiv.appendChild(h('div', {style: 'font-family:Georgia;font-size:13px;margin-bottom:6px'}, (window.isEnglish && window.isEnglish() ? '\uD83D\uDCCA Scientific benefit' : '\uD83D\uDCCA B\u00e9n\u00e9fice \u00e9tudi\u00e9')));
...
function generateCyclingPlan(level, days) {
 var _tCyc = window.isEnglish && window.isEnglish();
 var phases = [
 { name: (_tCyc ? 'Aerobic base' : 'Base aérobie'), weeks: [1, 2], color: '#1A3A6A', focus: (_tCyc ? 'Build the Z2 engine — quality over quantity' : 'Développer le moteur Z2 — qualité > quantité') },
 { name: (_tCyc ? 'Development' : 'Développement'), weeks: [3, 4, 5], color: '#7A3B0E', focus: (_tCyc ? 'Introduce tempo and sweet spot' : 'Introduire tempo et sweet spot') },
 { name: (_tCyc ? 'Specific' : 'Spécifique'), weeks: [6, 7], color: '#7A1F1F', focus: (_tCyc ? 'Threshold and VO2max intervals' : 'Intervalles seuil et VO2max') },
 { name: (_tCyc ? 'Taper' : 'Affûtage'), weeks: [8], color: '#3E5C3A', focus: (_tCyc ? 'Volume reduction — maintain intensity' : 'Réduction volume — maintien intensité') }
 ];
 ...
 var isSpecific = phase.name === 'Spécifique' || phase.name === 'Specific';
...
function renderCyclingOnboarding(p) {
 ...
 p.appendChild(h('h1', {html: (window.isEnglish && window.isEnglish() ? 'Your cycling<br><em>plan</em>' : 'Votre plan<br><em>vélo</em>')}));
 ...
 p.appendChild(h('div', {'class': 'section-label'}, (window.isEnglish && window.isEnglish() ? 'Current FTP in watts (optional — leave blank if unknown)' : 'FTP actuel en watts (optionnel — laissez vide si inconnu)')));
 ...
 p.appendChild(h('div', {'class': 'num-hint'}, (window.isEnglish && window.isEnglish() ? 'Functional Threshold Power — max sustainable power for 1h' : 'Functional Threshold Power — puissance max maintenable sur 1h')));
 ...
 zonesCard.appendChild(h('div', {style: 'font-family:Georgia;font-size:13px;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'Your power zones' : 'Vos zones de puissance')));
 ...
 p.appendChild(h('div', {'class': 'section-label', style: 'margin-top:16px'}, (window.isEnglish && window.isEnglish() ? 'Current average speed (km/h)' : 'Vitesse moyenne actuelle (km/h)')));
 ...
 p.appendChild(h('div', {'class': 'section-label', style: 'margin-top:16px'}, (window.isEnglish && window.isEnglish() ? 'Target elevation per ride' : 'Dénivelé cible par sortie')));
 var _tRelief = window.isEnglish && window.isEnglish();
 var reliefs = [
 { id: 'flat', name: (_tRelief ? 'Flat' : 'Plat'), desc: '< 500m D+/100km' },
 { id: 'rolling', name: (_tRelief ? 'Rolling' : 'Vallonné'), desc: '500-1000m D+/100km' },
 { id: 'mountainous', name: (_tRelief ? 'Mountainous' : 'Montagneux'), desc: '> 1000m D+/100km' }
 ];
 ...
 if (!ok) p.appendChild(h('div', {'class': 'field-error', style: 'text-align:center;margin-bottom:8px'}, (window.isEnglish && window.isEnglish() ? 'Choose your level and goal to continue.' : 'Choisissez votre niveau et votre objectif pour continuer.')));
 p.appendChild(h('button', {'class': 'btn-primary', disabled: !ok, onclick: function(){
 if (!ok) return;
...
function renderCyclingProgram(p) {
 ...
 var goalNames = (window.isEnglish && window.isEnglish()) ? { weightloss: 'Weight loss', endurance: 'Base endurance', competitive: 'Competitive', granfondo: 'Gran Fondo', triathlon: 'Triathlon' } : { weightloss: 'Perte de poids', endurance: 'Endurance de base', competitive: 'Sportif compétitif', granfondo: 'Gran Fondo', triathlon: 'Triathlon' };
 var bikeNames = { road: 'Route ', vtt: 'VTT ', indoor: 'Indoor ', gravel: 'Gravel ' };

 p.appendChild(h('div', {'class': 'eyebrow'}, 'Cyclisme'));
 p.appendChild(h('h1', {html: (goalNames[S.cyclingGoal] || 'Cycling') + '<br><em>' + (window.isEnglish && window.isEnglish() ? '8-week plan' : 'Plan 8 semaines') + '</em>'}));
 ...
 (S.cyclingFTP ? ' · ' + (window.isEnglish && window.isEnglish() ? 'Current FTP: ' : 'FTP actuel : ') + S.cyclingFTP + 'W' : '') +
 (ftpTarget ? ' \u2192 ' + (window.isEnglish && window.isEnglish() ? 'target: ~' : 'cible : ~') + ftpTarget + 'W' : '') +
 (speedTarget ? ' · ' + (window.isEnglish && window.isEnglish() ? 'Target speed: ~' : 'Vitesse cible : ~') + speedTarget + ' km/h' : '');
 ...
 var med = S.muscuMedical || {};
 var warnings = [];
 if (med.hypertension) warnings.push(window.isEnglish && window.isEnglish() ? ' HTN: avoid Z4/Z5 (no Valsalva) — stay in Z1-Z3 max' : ' HTA : évitez Z4/Z5 (Valsalva interdit) — restez en Z1-Z3 maximum');
 if (med.herniaDisc || med.lowerBack) warnings.push(window.isEnglish && window.isEnglish() ? ' Herniated disc: aerodynamic position not recommended — upright bike, raised handlebar' : ' Hernie discale : position aérodynamique déconseillée — vélo droit recommandé, guidon surélevé');
 if (med.knees || med.meniscus) warnings.push(window.isEnglish && window.isEnglish() ? ' Knee/Meniscus: high cadence (>90 RPM) recommended — avoid big gear efforts' : ' Gonarthrose / Ménisque : cadence élevée (>90 RPM) recommandée — évitez les grosses relances');
 if (getAge() >= 50) warnings.push(window.isEnglish && window.isEnglish() ? ' 50+: minimum 15 min warm-up required, Z1-Z3 priority, 48h recovery between intense sessions' : ' 50+ : échauffement 15 min minimum obligatoire, zones 1-3 prioritaires, récupération 48h entre séances intenses');
 if (warnings.length) {
 ...
 }}, zonesToggle ? (window.isEnglish && window.isEnglish() ? '▲ Hide FTP zones' : '▲ Masquer les zones FTP') : (window.isEnglish && window.isEnglish() ? '▼ Show FTP zones' : '▼ Voir les zones FTP')));
 if (zonesToggle) {
 ...
 ftpBox.appendChild(h('div', {style: 'font-family:Helvetica Neue,Arial,sans-serif;font-size:13px;color:var(--grey);line-height:1.6'}, (window.isEnglish && window.isEnglish() ? 'Perform a maximal 20-minute effort at steady state. Estimated FTP ≈ average power over 20 min × 0.95. Enter it in settings to show your personal zones.' : 'Effectuez un effort maximal de 20 minutes en régime stable. Votre FTP estimée ≈ puissance moyenne sur 20 min × 0,95. Renseignez-la dans la configuration pour afficher vos zones personnalisées.')));
 ...
 p.appendChild(h('div', {style: 'height:12px'}));
 p.appendChild(h('button', {'class': 'btn-back', onclick: function() { S.sStep = 22; window.render(); }, html: backArrow + (window.isEnglish && window.isEnglish() ? 'Edit settings' : 'Modifier la configuration')}));
 appendNutritionModeCTA(p);
}
...
function renderCalisthenicsProgram(content) {
 ...
 try {
  ...
  content.appendChild(h('div', {'class': 'card'}, h('div', {style: 'color:red;font-size:13px'}, (window.isEnglish && window.isEnglish() ? 'Cannot display program — please retry or reload. (' + e.message + ')' : 'Impossible d\'afficher le programme — réessayez ou rechargez la page. (' + e.message + ')'))));
  return;
 }
...
window.exportSportPDF = function() {
  ...
  try {
    ...
    doc.text((window.isEnglish && window.isEnglish() ? 'Weight Training Program \u2014 Week ' : 'Programme Musculation \u2014 Semaine ') + (S.muscuWeek || 1), M, 24);
    ...
    doc.save((window.isEnglish && window.isEnglish() ? 'weighttraining-program-week' : 'programme-musculation-sem') + (S.muscuWeek || 1) + '.pdf');
