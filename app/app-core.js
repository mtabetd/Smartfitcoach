...
function filterRecipes(pool,type){
  ...
  if(s.regime===2)r=r.filter(function(x){var i=_ri(x);return!(/poulet|boeuf|bœuf|veau|dinde|agneau|kefta|steak(?!\s+de\s+(?:tofu|seitan|tempeh|soja|l[eé]gumes|v[eé]g[eé]tal))|saumon|thon|crevette|cabillaud|dorade|daurade|sardine|maquereau|poisson|sole|filet de bar|branzino|moules|poulpe|canard|lapin|lièvre|merguez|gambas|lotte|morue|foie|rognon|cœur|coeur|anchois|truite|colin|\bbar\b|lieu noir|mahi.?mahi|merlu|tilapia|hareng|mulet|pageot|vivaneau|saint-pierre|lingue|grondin|rascasse|porc(?!ini)|cochon|lard|lardon|bacon|pancetta|prosciutto|chorizo|pepperoni|saucisson|saucisse|andouille|boudin|rillette|jambon|charcuterie/).test(i)});
...
return{recipe:r,score:calScore+macroScore+diversityPenalty+sportTypeBonus+favBonus+stalenessScore+unseenBonus};});scored.sort(function(a,b){return a.score-b.score});if(window.SFC_RECIPE_DEBUG){try{var _dbgTop=scored.slice(0,3).map(function(x){return x.recipe.n+'(score:'+Math.round(x.score)+')';});console.log('[SFCVariety Debug] Pool:'+pool.length+' Available:'+av.length+' TopPicks:',_dbgTop.join(', '));}catch(_de){}}var picked=window.SFCVariety?window.SFCVariety.weightedPick(scored,12):(function(){var top=scored.slice(0,Math.min(5,scored.length));return top[Math.floor(Math.random()*top.length)].recipe;})();if(picked){used.add(picked.n);var pc=getRecipeProtein(picked);if(pc){if(dayProteins)dayProteins.push(pc);if(weekProtBudget)weekProtBudget[pc]=(weekProtBudget[pc]||0)+1;}}return picked||{n:'Repas libre',k:targetK,p:Math.round(targetK*0.3/4),g:Math.round(targetK*0.4/4),l:Math.round(targetK*0.3/9),f:0,lv:1,i:'',st:[],w:0,tags:[]}}
