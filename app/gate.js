(function(){
  ...
  function checkGate(){
    var stored;
    try { stored=localStorage.getItem(GATE_KEY); } catch(e) { stored=null; }
    if(stored===GATE_HASH){unlock();return;}
    if(stored===GATE_HASH_LEGACY){ try{localStorage.removeItem(GATE_KEY);}catch(e){} }
